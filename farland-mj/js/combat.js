...
window.CombatEngine = (function () {
  ...
  function build(scene) {
    ...
    return { sig: sig(scene), round: 1, phase: "player", tab: "atk",
      ...
      mark: null, markName: null, hidden: false, dodge: false,
      initiative: null, log: [] };
  }
  ...
  function render() {
      ...
      return `<div class="cb-foe ${e.defeated ? "ko" : ""} ${sel ? "sel" : ""}" data-tgt="${i}">
        ...
        <div class="cb-foe-main">
          <div class="cb-foe-name">${e.defeated ? "💀 " : ""}${esc(e.nom)} <span class="enemy-ac">🛡️ ${e.ac}</span>
            ${S.mark === i ? `<span class="mark-badge">🎯</span>` : ""}</div>
    ...
    if (S.phase === "won") {
      ...
      pane = `<div class="cb-end lost">
        ...
        <p>Tout devient noir… puis une mâchoire se referme sur ton col : Fenn te traîne à l'abri. Tu te réveilles, meurtrie mais vivante.</p>
        <button class="btn-accent btn-block" id="cb-exit-lost">Se relever à 1 PV ▶</button></div>`;
    } else {
  ...
  function heroAttack(ai) {
    ...
    const hero = D.heroSheet(); const a = hero.attaques[ai];
    const e = needTarget(); if (!e) return;
    const t = S.target;
    const mode = S.hidden ? "adv" : D.d20mode();
    ...
    if (hitOk) {
      ...
      log(`⚔️ ${a.nom} → ${e.nom} : ${r.total}${r.crit ? " ⭐CRIT" : ""} vs CA ${e.ac} — TOUCHÉ, ${dmg} dégâts${extra}${e.defeated ? " · VAINCU 💀" : ""}`, "ok");
  ...
  function maneuver(m) {
    ...
    if (m === "shove") {
      ...
      S.turn.action = false;
      if (mine.total >= theirs.total) log(`🤜 Bousculade ${mine.total} vs ${theirs.total} : ${e.nom} est À TERRE (avantage au corps-à-corps contre lui) !`, "ok");
      else log(`🤜 Bousculade ${mine.total} vs ${theirs.total} : ${e.nom} tient bon.`, "ko");
    }
    if (m === "flee") {
